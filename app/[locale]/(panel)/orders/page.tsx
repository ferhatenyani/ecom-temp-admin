...
/**
 * The orders list. The most-used screen in the panel — designed for a phone held
 * in one hand in a stockroom.
 *
...
export default async function OrdersPage({
  ...
  const [initial, geography] = await Promise.all([
    acFetch(orderList, session, "/orders", {
      query: { per_page: PER_PAGE, page: 1, status: query.status, search: query.search },
    }).catch((error: unknown) => {
