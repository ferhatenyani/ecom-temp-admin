 ...
 * (there the order's address is on the page behind it) and this not one. Size
 * `md` (560): a name, at most eleven criterion rows and a picker, none of which
 * carries a long place name the way the shipping rule form's selects do.
 *
      ...
      <div className="flex flex-col gap-4">
        ...
        <section className="flex flex-col gap-3">
            ...
            <div key={key} className="flex items-end gap-2">
              <div className="min-w-0 flex-1">
                <TextField
                  ...
                  inputMode={
                    isSegmentCriterion(key) && CRITERION_KIND[key] === "date"
                      ? "text"
                      : "decimal"
                  }
                  isolate
          ...
          {/*
            ...
            cannot act. The picker offers only the eleven the API supports, which
            is what keeps the seven refused-by-name criteria (`consent`, `sql`,
            `email_contains` and four more) unreachable from this screen.
          */}
          {available.length > 0 ? (
            <Select<string>
